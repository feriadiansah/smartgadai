                            ...
                            <tr class="hover:bg-slate-50 transition-colors">
                                <td class="px-6 py-4">
                                    ...
                                    <div class="flex items-center gap-2 mt-1">
                                        ...
                                        <!-- Tombol WA Satuan (Format Resmi Pegadaian) -->
                                        @php
                                            $noHpIndividu = $pinjaman->nasabah->nomor_hp;
                                            $linkWaIndividu = '#';

                                            if ($noHpIndividu) {
                                                // Bersihkan nomor HP dan ubah ke 62
                                                $noHpIndividu = preg_replace('/[^0-9]/', '', $noHpIndividu);
                                                if (substr($noHpIndividu, 0, 1) == '0') {
                                                    $noHpIndividu = '62' . substr($noHpIndividu, 1);
                                                }

                                                // FORMAT PESAN RESMI PEGADAIAN
                                                $pesanIndividu =
                                                    'Yth Bpk/Ibu ' .
                                                    strtoupper($pinjaman->nasabah->nama_lengkap) .
                                                    "\n\n";
                                                $pesanIndividu .= 'Angsuran Kredit Produk ' . $pinjaman->produk . "\n";
                                                $pesanIndividu .= 'No Kontrak; ' . $pinjaman->no_kredit . "\n";
                                                $pesanIndividu .=
                                                    'Akan Jatuh Tempo Tanggal: ' .
                                                    \Carbon\Carbon::parse($pinjaman->tgl_jatuh_tempo)->translatedFormat(
                                                        'd M Y',
                                                    ) .
                                                    "\n\n";
                                                $pesanIndividu .=
                                                ...
                                                $pesanIndividu .=
                                                    "Transaksi Semakin Mudah; https://tring.onelink.me/rIEN/infoPegadaian\n\n";
                                                ...
                                                $linkWaIndividu =
                                                    ...
                                                    $noHpIndividu .
                                                    '&text=' .
                                                    urlencode($pesanIndividu);
                                            }
                                        @endphp

                                        @if ($pinjaman->nasabah->nomor_hp)
                                            <a href="{{ $linkWaIndividu }}" target="_blank"
                                ...
                                <td class="px-6 py-4 font-mono text-slate-600 font-medium">{{ $pinjaman->no_kredit }}
                                </td>
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="showModalWA" x-transition:enter="ease-out duration-300"
                    ...
                    <div class="bg-slate-50 px-6 py-4 max-h-[60vh] overflow-y-auto">
                        @if (count($dataReminder) > 0)
                            ...
                            <div class="space-y-3">
                                @foreach ($dataReminder as $index => $reminder)
                                    <!-- Tombol WA Antrean Massal (SUDAH DIGANTI FORMAT PEGADAIAN) -->
                                    @php
                                        $noHpQueue = $reminder->nasabah->nomor_hp;
                                        $linkWaQueue = '#';

                                        if ($noHpQueue) {
                                            // Bersihkan nomor HP dan ubah ke 62
                                            $noHpQueue = preg_replace('/[^0-9]/', '', $noHpQueue);
                                            if (substr($noHpQueue, 0, 1) == '0') {
                                                $noHpQueue = '62' . substr($noHpQueue, 1);
                                            }

                                            // FORMAT PESAN RESMI PEGADAIAN
                                            $pesanQueue =
                                                'Yth Bpk/Ibu ' . strtoupper($reminder->nasabah->nama_lengkap) . "\n\n";
                                            $pesanQueue .= 'Angsuran Kredit Produk ' . $reminder->produk . "\n";
                                            $pesanQueue .= 'No Kontrak; ' . $reminder->no_kredit . "\n";
                                            $pesanQueue .=
                                                'Akan Jatuh Tempo Tanggal: ' .
                                                \Carbon\Carbon::parse($reminder->tgl_jatuh_tempo)->translatedFormat(
                                                    'd M Y',
                                                ) .
                                                "\n\n";
                                            $pesanQueue .= "Abaikan pesan ini jika sudah melakukan pembayaran\n\n";
                                            $pesanQueue .=
                                                "Transaksi Semakin Mudah; https://tring.onelink.me/rIEN/infoPegadaian\n\n";
                                            $pesanQueue .= "Info\nPEGADAIAN KEBAYORAN BARU";
                                    ...
                                    <div class="bg-white border border-slate-200 p-4 rounded-xl flex justify-between items-center shadow-sm hover:border-green-300 transition-colors"
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <!-- Tombol Eksekusi WA (VARIABEL YANG BENAR) -->
                                            <a href="{{ $linkWaQueue }}" target="_blank" @click="dikirim = true"
                        ...
                        @else
                            <!-- Jika tidak ada yang perlu ditagih -->
                    ...
                    <div class="bg-white px-6 py-4 border-t border-slate-200 flex justify-end">
                        <button @click="showModalWA = false"
                            class="bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold py-2 px-6 rounded-lg transition-colors">
                            Tutup
                        </button>
                    </div>
                </div>
